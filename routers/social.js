...
router.post('/google', passport.authenticate('google-token', {session: false}), async(req, res, next) => {
    try{
        ...
        // 檢查用戶是否已存在
        const [googleUser, field] = await connection.execute('SELECT * FROM goals.member WHERE email=?', [email]);
        ...
        // 加入新用戶資料到資料庫中
        await connection.execute('INSERT INTO goals.member (email, username, password, valid, verifyString) VALUE (?, ?, ?, ?, ?)', [email, name, "socialMedia", verified_email, provider]); 
        const [newGoogleUser] = await connection.execute('SELECT * FROM goals.member WHERE email=?', [email]);
        const googleMember = {
