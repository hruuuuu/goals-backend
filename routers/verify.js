...
router.post('/resend', async(req, res, next) => {
    ...
    const updateData = await connection.execute('UPDATE goals.member SET verifyString=?, timeStamp=? WHERE email=?', [verifyCode, joinTime, email]);
...
router.post('/forget', async(req, res, next) => {
    ...
    const updateData = await connection.execute('UPDATE goals.member SET verifyString=? WHERE email=?', [verifyCode, email]);
    console.log(updateData)
    sendMail();
    alert("已寄出重設密碼信件");
    res.json({
...
router.post('/reset/', async(req, res, next) => {
    ...
    try{
        const [passwordCheck] = await connection.execute('SELECT * FROM goals.member WHERE email=?', [email]);
        ...
        const checkPassword = await argon2.verify(passwordCheck[0].password, password);
        if(checkPassword){
            alert("密碼不可與之前的相同");
            return res.status(400).json({
                msg: "密碼不可與之前的相同"
        ...
        const [updatePassword] = await connection.execute('UPDATE goals.member SET password = ? WHERE email = ?', [newPassword, email]);
